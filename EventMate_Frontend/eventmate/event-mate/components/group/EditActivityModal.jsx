...
const EditActivityModal = ({ isOpen, onClose, onSubmit, activity: initialActivity }) => {
    ...
    useEffect(() => {
      if (initialActivity) {
        const [date, time] = initialActivity.schedule.split('T');
        setActivity({
          category: initialActivity.category || '',
          content: initialActivity.content || '',
          schedule: time.slice(0, 5),
          time: time.slice(0, 5), // Extract only the HH:mm part
          status: initialActivity.status || 0,
        });
      }
    }, [initialActivity]);
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    type="time"
                    value={activity.time || ''}
                    onChange={(e) => setActivity({ ...activity, time: e.target.value,schedule:e.target.value })}
            ...
            <div>
              ...
              <select
                ...
                onChange={(e) => setActivity({ ...activity, status: parseInt(e.target.value) })}
                className="w-full px-4 py-2.5 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              >
